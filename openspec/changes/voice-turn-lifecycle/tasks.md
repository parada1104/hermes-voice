...
- [x] 1.7 **M3 (manual, blocks default-on)**: on the real speaker/mic pair, measure the RMS threshold and sustained-frame count that separate Robert's voice from Hermes' own TTS bleeding back through the mic, the same way `vad.js:14-16` recorded its 0.02 (silent-room peak 0.00392, voice p50 0.05585). Record the result in `mediciones.md`'s M3 section (currently NOT RUN). **DONE 2026-08-30**: corrida en el paso 4 nuevo de `app/ui/diagnostico-audio.html` (MacBook M1, Chrome 150, parlante+mic internos). Eco solo 105ms de vozMs contra los 300 que exige el corte (2.9x de holgura); voz encima 4197ms (14x). Separacion sostenida 9.9x (voz p50 0.02094 vs eco p95 0.00211). PASA con las constantes actuales. El pico aislado de 0.05015 es UN frame de 77 — por eso el corte exige voz sostenida. Registrada en mediciones.md.
